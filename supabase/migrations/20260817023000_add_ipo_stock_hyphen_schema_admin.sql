create schema if not exists "ipo-stock";
grant usage on schema "ipo-stock" to service_role;

create or replace function "ipo-stock".schema_list_tables()
returns jsonb
language sql
stable
security definer
...
as $$
  ...
  from (
    select
      ...
      jsonb_build_object(
        'name', c.relname,
        'kind', case c.relkind when 'v' then 'view' else 'table' end,
        'columns', coalesce((
          select jsonb_agg(
            jsonb_build_object(
              'name', a.attname,
...
create or replace function "ipo-stock".schema_add_column(
  p_table text,
  p_column text,
  p_type text,
...
begin
  ...
  execute format(
    'alter table "ipo-stock".%I add column %I %s%s',
    p_table,
    p_column,
    p_type,
...
create or replace function "ipo-stock".schema_drop_column(
  p_table text,
  p_column text
)
returns jsonb
...
begin
  ...
  execute format(
    'alter table "ipo-stock".%I drop column %I',
    p_table,
    p_column
  );
  perform pg_catalog.pg_notify('pgrst', 'reload schema');
...
create or replace function "ipo-stock".schema_list_routines()
returns jsonb
language sql
stable
security definer
...
revoke all on function "ipo-stock".schema_list_tables() from public, anon, authenticated;
revoke all on function "ipo-stock".schema_add_column(text, text, text, boolean) from public, anon, authenticated;
revoke all on function "ipo-stock".schema_drop_column(text, text) from public, anon, authenticated;
revoke all on function "ipo-stock".schema_list_routines() from public, anon, authenticated;
grant execute on function "ipo-stock".schema_list_tables() to service_role;
